...
var hasFloat = new RegExp(floatRegex.source);
var hasColor = new RegExp(colorRegex.source, "i");
function test(v) {
...
function asRGBA(color2) {
  ...
  if (!type) {
    warning(false, `'${color2}' is not an animatable color. Use the equivalent color code instead.`, "color-not-animatable");
    return false;
  }
...
function spring(optionsOrVisualDuration = springDefaults.visualDuration, bounce = springDefaults.bounce) {
  ...
  const generator = {
     ...
     */
    retarget: (keyframes2, newVelocity) => {
      s.target = keyframes2[keyframes2.length - 1];
      s.delta = s.target - keyframes2[0];
    ...
    next: (t) => {
      ...
      if (!isResolvedFromDuration) {
        ...
        state.done = Math.abs(currentVelocity) <= s.restSpeed && Math.abs(s.target - current2) <= s.restDelta;
      } else {
        state.done = t >= duration;
      }
      state.value = state.done ? s.target : current2;
      return state;
    },
    toString: () => {
      const easingDuration = Math.min(calcGeneratorDuration(generator), maxGeneratorDuration);
      const easing = generateLinearEasing((progress2) => generator.next(easingDuration * progress2).value, easingDuration, 30);
...
// node_modules/motion-dom/dist/es/animation/utils/notify-inspector.mjs
...
function notifyAnimationStart(animation, options, transition) {
  const notify2 = globalThis.__MOTION_INSPECT__;
  if (!notify2)
    return;
  try {
    notify2({
...
var WithPromise = class {
  ...
  get finished() {
    if (!this._finished) {
      this._finished = this.isResolved ? Promise.resolve() : new Promise((resolve) => {
...
var JSAnimation = class extends WithPromise {
  ...
  initAnimation() {
    ...
    const generator = generatorFactory(keyframes$1 === options.keyframes ? options : { ...options, keyframes: keyframes$1 });
...
function removeNonTranslationalTransform(visualElement) {
  ...
  nonTranslationalTransformKeys.forEach((key) => {
    ...
    if (value !== void 0) {
      const current2 = value.get();
      const reset = key.startsWith("scale") ? 1 : 0;
      if (current2 === reset)
        return;
      removedTransforms.push([key, current2]);
      value.set(reset);
    }
  });
...
var positionalValues = {
  ...
  // Transform
  x: ({ transform: transform2 }) => parseValueFromTransform(transform2, "x"),
  y: ({ transform: transform2 }) => parseValueFromTransform(transform2, "y")
};
...
function readOrigin(value, name, target) {
  if (typeof value === "string") {
    if (isNumericalString(value) || isZeroValueString(value)) {
      ...
      return getAnimatableNone2(name, target);
    }
  }
  return value ?? void 0;
}
...
var AsyncMotionValueAnimation = class extends WithPromise {
  constructor(options) {
    ...
    const { keyframes: keyframes2, name, motionValue: motionValue2, element } = options;
...
var MotionValue = class {
  ...
  notifyChange() {
    const { current: current2, changeSubscriber } = this;
    if (changeSubscriber) {
      changeSubscriber(current2);
    } else {
      this.events.change?.notify(current2);
    }
  }
...
function makeNoneKeyframesAnimatable(unresolvedKeyframes, noneKeyframeIndexes, name) {
  ...
  while (i < unresolvedKeyframes.length && !animatableTemplate) {
    ...
    if (typeof keyframe === "string" && !invalidTemplates.has(keyframe) && hasComplexValues(keyframe)) {
...
function animateValues(getValue, keyframes2, transition = {}, element) {
  ...
  for (const key in keyframes2) {
    ...
    const current2 = value.get();
    if (current2 !== void 0 && !value.isAnimating() && !Array.isArray(target) && target === current2 && !velocity) {
...
function createSelectorEffect(subjectEffect) {
  ...
  const { test: test2, read, get, flush, state } = subjectEffect;
...
function buildTransform(state) {
  let transform2 = "";
  const { transformKeys: keys2 = [], transformValues: values = {} } = state;
  for (let i = 0; i < keys2.length; i++) {
    ...
    if (parsed !== (key.startsWith("scale") ? 1 : 0)) {
      transform2 += (transform2 && " ") + (openers[key] || (openers[key] = (translateAlias[key] || key) + "(")) + getValueAsType(value, transformValueTypes[key]) + ")";
    }
  }
  const pathRotation = state.get("pathRotation")?.get();
  if (pathRotation) {
    transform2 += (transform2 && " ") + "rotate(" + getValueAsType(pathRotation, transformValueTypes.pathRotation) + ")";
  }
...
var addStyleValue = (element, state, key, value) => {
  ...
  if (transformProps.has(key)) {
    if (key !== "pathRotation") {
      const keys2 = state.transformKeys ?? (state.transformKeys = []);
      (state.transformValues ?? (state.transformValues = {}))[key] = value;
      if (!keys2.includes(key)) {
        keys2.push(key);
        keys2.sort((a, b) => transformPropOrder.indexOf(a) - transformPropOrder.indexOf(b));
      }
    }
...
var styleSubjectEffect = createEffect(addStyleValue, {
  test: isStyleSubject,
  read: readStyleValue
});
var styleEffect = createSelectorEffect(styleSubjectEffect);
...
var svgSubjectEffect = createEffect(addSVGValue, {
  test: isSVGElement,
  read: readSVGValue
});
var svgEffect = createSelectorEffect(svgSubjectEffect);
...
function handOffElementState(element, visualElement) {
  ...
  if (!state)
    return;
  const { transformKeys: transformKeys2 } = state;
  state.release().forEach((value, key) => {
    const derived = key === "transformBox" || key === "transform" && transformKeys2?.length;
...
// node_modules/motion-dom/dist/es/animation/FollowAnimation.mjs
...
var FollowAnimation = class extends WithPromise {
  ...
  retarget(keyframes2, velocity) {
    ...
    if (generator.retarget) {
      generator.retarget(keyframes2, velocity);
    } else {
      this.generator = this.factory(options);
    }
  }
  tick(timestamp) {
    ...
    const { delay: delay2 = 0, onUpdate, onPlay } = options;
    const elapsed = Math.round(timestamp - this.startTime) - delay2;
    ...
    if (retargeted) {
      this.hasNextTarget = false;
      const { keyframes: keyframes2 } = options;
      keyframes2[0] = value;
      keyframes2[1] = this.nextTarget;
      this.retarget(keyframes2, this.nextVelocity ?? this.getGeneratorVelocity());
    }
  ...
  readValue(key, target) {
    ...
    if (value !== void 0 && value !== null) {
      if (typeof value === "string" && (isNumericalString(value) || isZeroValueString(value))) {
         ...
         */
        typeof value !== "number" && !complex.test(value) && complex.test(target)
      ) {
...
function createProjectionNode({ attachResizeListener, defaultParent, measureScroll, checkIsScrollRoot, resetTransform }) {
  return class ProjectionNode {
    ...
    applyTransformsToTarget() {
      const lead = this.getLead();
      const { targetWithTransforms, layout: layout2, latestValues } = lead;
      let { target } = lead;
